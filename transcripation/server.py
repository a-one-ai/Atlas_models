...
from transcription_model import transcribeLink,transcribeVideo,transcribeAudio
import os
...
def process_video():
    ...
    if video and video.filename:
        return process_file(video, UPLOAD_VIDEOS_FOLDER, transcribeVideo)
...
def process_youtube_link():
    ...
    if video_link:
        try:
            result = transcribeLink(video_link)
            return jsonify({"data": result})
        except Exception as e:
            print(f"Error {type(e).__name__} occurred: {e}")
